...
public class FileUtils {
     ...
     * @return
     */
    public static String getThumPath(ContentResolver resolver, int id) {
        String thumbPath = "";
        ...
        Cursor thumbCursor = resolver.query(
                ...
                thumbColumns,
                MediaStore.Video.Thumbnails.VIDEO_ID + "=" + id,//根据视频id去缩略图表里查询对应的缩略图
                null,
                null);
        if (thumbCursor != null) {
            if (thumbCursor.moveToFirst()) {
                thumbCursor.getColumnCount();
                thumbPath = thumbCursor.getString(thumbCursor.getColumnIndex(MediaStore.Video.Thumbnails.DATA));//获取路径column的值
            }
            thumbCursor.close();
        }
        return thumbPath;
