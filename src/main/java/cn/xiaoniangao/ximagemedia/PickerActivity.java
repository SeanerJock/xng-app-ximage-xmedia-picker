...
import cn.xiaoniangao.ximagemedia.adapter.FolderAdapter;
import cn.xiaoniangao.ximagemedia.adapter.MediaGridAdapter;
import cn.xiaoniangao.ximagemedia.adapter.SpacingDecoration;
import cn.xiaoniangao.ximagemedia.data.DataCallback;
import cn.xiaoniangao.ximagemedia.data.ImageLoader;
import cn.xiaoniangao.ximagemedia.data.MediaLoader;
import cn.xiaoniangao.ximagemedia.data.VideoLoader;
...
import cn.xiaoniangao.ximagemedia.utils.FileUtils;
import cn.xiaoniangao.ximagemedia.utils.ScreenUtils;
...
public class PickerActivity extends AppCompatActivity implements DataCallback, View.OnClickListener {
    ...
    void createAdapter() {
        ...
        fullToast = argsIntent.getStringExtra(PickerConfig.CHOOSE_FULL_TOAST);
        gridAdapter = new MediaGridAdapter(medias, this, select, maxSelect, maxSize, fullToast);
    ...
    void setView(ArrayList<Folder> list) {
        ...
        gridAdapter.setOnItemClickListener(new MediaGridAdapter.OnRecyclerViewItemClickListener() {
            ...
            @Override
            public void onPreviewClick(Media data) {
                goToPreviewActivity(data);
            }
        });
    }
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.btn_back) {
            ...
            goToPreviewActivity(null);
        }
    }

    private void goToPreviewActivity(Media data) {
        ...
        if (data == null) {
            ...
            intent.putExtra(PickerConfig.FIRST_PRE_ITEM, data);
            intent.putExtra(PickerConfig.PRE_RAW_LIST, gridAdapter.getMedias());
        }
        intent.putExtra(PickerConfig.PRE_SELECT_LIST, gridAdapter.getSelectMedias());
        intent.putExtra(PickerConfig.CHOOSE_FULL_TOAST, fullToast);
    ...
    public void done(ArrayList<Media> selects) {
        ...
        String uriType = argsIntent.getStringExtra(PickerConfig.URI_TYPE);
        if (TextUtils.isEmpty(uriType)) {
            uriType = PickerConfig.FILE_TYPE;
        }
        Intent intent = new Intent();
        if (selects != null && selects.size() > 0) {
            for (Media media : selects) {
                ResultModel resultModel = new ResultModel();
                resultModel.setMediaType(media.mediaType);
                if (media.mediaType == 1) {//图片类型
                    String path = "";
                    if (uriType.equals(PickerConfig.FILE_TYPE)) {
                        path = Uri.parse("file://" + media.path).toString();
                    } else {
                        path = UriUtils.getImageContentUri(PickerActivity.this, media.path).toString();
                    }
                    resultModel.setUriPath(path);
                } else if (media.mediaType == 3) {//视频类型
                    String path = "";
                    String thumPath = FileUtils.getThumPath(PickerActivity.this.getContentResolver(), media.id);
                    if (uriType.equals(PickerConfig.FILE_TYPE)) {
                        path = Uri.parse("file://" + media.path).toString();
                        thumPath = Uri.parse("file://" + thumPath).toString();
                    } else {
                        path = UriUtils.getVideoContentUri(PickerActivity.this, media.path).toString();
                        thumPath = UriUtils.getThumImageContentUri(PickerActivity.this, thumPath).toString();
                    }
                    resultModel.setUriPath(path);
                    resultModel.setThumPath(thumPath);
                }
                resultModels.add(resultModel);
            }
        }
        intent.putParcelableArrayListExtra(PickerConfig.EXTRA_RESULT, resultModels);
